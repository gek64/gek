...
func InternalDownloaderWithFolder(url string, outputFolder string) (err error) {
	...
	// header中无文件路径则使用默认路径
	if fileName == "" {
		fileName = gMath.RandStringRunes(5)
	}
...
func ExternalDownloaderWithFolder(url string, outputFolder string) (err error) {
	...
	switch downloader {
	case "aria2c":
		err = gExec.Run(exec.Command("aria2c", "-s", "16", "-x", "16", url, "--allow-overwrite=true", "-d", outputFolder))
		if err != nil {
			return err
		}
	case "wget":
	...
	case "curl":
		err = gExec.Run(exec.Command("curl", "--create-dirs", "--output-dir", outputFolder, "-LOJ", url))
		if err != nil {
			return err
		}
	case "fetch":
		...
		// 下载文件
		err = gExec.Run(exec.Command("fetch", url))
		if err != nil {
			return err
		}
	default:
...
func ExternalDownloaderWithFilePath(url string, outputFile string) (err error) {
	...
	switch downloader {
	case "aria2c":
		err = gExec.Run(exec.Command("aria2c", "-s", "16", "-x", "16", url, "--allow-overwrite=true", "-o", outputFile))
		if err != nil {
			return err
		}
	case "wget":
		err = gExec.Run(exec.Command("wget", url, "-O", outputFile))
		if err != nil {
			return err
		}
	case "curl":
		err = gExec.Run(exec.Command("curl", "--create-dirs", "-Lo", outputFile, url))
		if err != nil {
			return err
		}
	case "fetch":
		err = gExec.Run(exec.Command("fetch", url, "-o", outputFile))
		if err != nil {
			return err
		}
	default:
