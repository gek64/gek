...
func GetDownloadLink(repo string, appMap map[string]string) (downloadLink string, err error) {
	...
	// 获取Github Release
	githubAPI, err := NewReleaseLatest(repo)
	if err != nil {
		return "", err
	}
	...
	part := appMap[keyPair]
	if part != "" {
		release := githubAPI.SearchRelease([]string{part}, []string{})
		if len(release) <= 0 {
			return "", fmt.Errorf("can not find the release for %s", keyPair)
		}
		downloadLink = release[0].BrowserDownloadURL
	} else {
